...
import { useParams } from 'react-router-dom';
 export default function Productos2 () {
  ...
  const { accesorios } = useParams(); // ID del producto desde la URL
  const fetchProducts = async () => {
    try {
      const response = await axios.get(`https://server-triton.vercel.app/categorias/${accesorios}`);
  ...
  products.map(product => (
        <div key={product.product_id} className="card-container">
          ...
          <p>No hay productos disponibles. en sugerencias</p>
        )}
    </Slider>
  </div>
  );
